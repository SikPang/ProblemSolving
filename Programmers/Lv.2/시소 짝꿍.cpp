...
unsigned long long solution(vector<int> weights) {
	...
	for (unordered_map<int, unsigned long long>::iterator iter = counter.begin(); iter != counter.end(); ++iter)
	{
		answer += iter->second * (iter->second - 1) / 2;
		if (counter.find(iter->first * 2) != counter.end())
			answer += counter[iter->first * 2];
		if (counter.find(iter->first * 3.0 / 2.0) != counter.end())
			answer += counter[iter->first * 3.0 / 2.0];
		if (counter.find(iter->first * 4.0 / 3.0) != counter.end())
			answer += counter[iter->first * 4.0 / 3.0];
	}
    return answer;
}
